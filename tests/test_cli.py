...
def test_cli_backtest_range_and_equity_export(tmp_path) -> None:
    csv_file = tmp_path / "bars.csv"
    base = datetime(2026, 3, 1, tzinfo=timezone.utc)
    rows = ["symbol,timeframe,ts,open,high,low,close,volume"]
    for i in range(20):
    ...
    assert not (tmp_path / "out2" / "report.csv").exists()
    assert (tmp_path / "out2" / "equity_curve.csv").exists()
    assert (tmp_path / "out2" / "drawdown_curve.csv").exists()


def test_cli_datasources() -> None:
    assert main(["datasources"]) == 0
